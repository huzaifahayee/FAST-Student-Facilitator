...
const TimetableManager = ({ user }) => {
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (!uploadUrl) {
      setUploadError("Please provide a valid Google Sheet CSV URL.");
      return;
    }
    setUploading(true);
    setUploadError(null);
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/timetable/upload?url=${encodeURIComponent(uploadUrl)}&ownerName=${encodeURIComponent(user.name)}&ownerEmail=${encodeURIComponent(user.email)}`, {
        method: 'POST'
      });
      if (!res.ok) {
