...
const BookExchange = ({ user }) => {
  ...
  const fetchBooks = async () => {
    try {
      let url = `http://localhost:8080/api/books?type=${activeTab}`;
      if (searchQuery) {
        url = `http://localhost:8080/api/books/search?query=${encodeURIComponent(searchQuery)}&type=${activeTab}`;
      }
        ...
        <div className="listings-grid">
          {books.length > 0 ? (
            ...
            <div className="empty-state glass-card">
              ...
              <p className="subtext">Check back later or post your own!</p>
            </div>
          )}
        </div>
      </section>
